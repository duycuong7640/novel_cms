...
use App\Services\Admins\PointService;
...
use Modules\Admins\Http\Requests\Point\CreateRequest;
use Modules\Admins\Http\Requests\Point\EditRequest;
...
class PointsController extends Controller
{
    ...
    public function __construct(PointService $pointService, CommonCategoryService $commonCategoryService)
    {
    ...
    public function index()
    {
        try {
            ...
            $data['list'] = $this->pointService->getList(['paginate' => \dataQuery::LIMIT]);
            ...
            return view('admins::points.index', ['data' => $data]);
        } catch (\Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('admins::points.create', ['data' => $data]);
        } catch (\Exception $e) {
            return !empty($e->getMessage()) ? abort('500') : abort(404);
        }
    }
    ...
    public function store(CreateRequest $request)
    {
        try {
            $_params = $request->all();
            if ($this->pointService->create($_params)) {
    ...
    public function edit($id)
    {
        try {
            ...
            $data['detail'] = $this->pointService->find($id);
            ...
            return view('admins::points.edit', ['data' => $data]);
        } catch (\Exception $e) {
            return !empty($e->getMessage()) ? abort('500') : abort(404);
        }
    }
    ...
    {
        try {
            $detail = $this->pointService->find($id);
            if (empty($detail->id)) return abort(404);

            $_params = $request->all();
            if ($this->pointService->update($detail->uuid, $_params)) {
                session()->flash('success', \adminNotify::SUCCESS);
                return redirect(route('admin.point.index', ['page' => (!empty($_GET['page']) ? $_GET['page'] : 1), 'parent_id' => (request()->has('parent_id') ? request()->get('parent_id') : '')]));
            } else {
                $errors = new MessageBag(['error' => \adminNotify::FAIL]);
                return back()->withInput($_params)->withErrors($errors);
            }
    ...
    public function status($id, $status)
    {
        try {
            ...
            if ($this->pointService->update($detail->uuid, [$status => ($detail->$status ? \dataCategory::DE_ACTIVE : \dataCategory::ACTIVE)])) {
                session()->flash('success', \adminNotify::SUCCESS);
            } else {
