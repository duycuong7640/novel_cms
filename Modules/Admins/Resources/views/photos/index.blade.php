                            ...
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data['list'] as $k=>$row)
                                    <tr>
                                        ...
                                        <td>{{ $row->title }}</td>
                                        <td>{{ $row->sort }}</td>
                                        <td>{{ dataPhoto::TYPE_NAME[$row->type] }}</td>
                                        ...
                                        <td>
                                            <a class="icon-form" title="edit"
                                               href="{{ route('admin.photo.edit', ['id' => $row->id, 'page' => $data['list']->currentPage(), 'parent_id' => (request()->has('parent_id') ? request()->get('parent_id') : '')]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active"
                                               href="{{ route('admin.photo.status', ['id' => $row->id, 'field' => 'status']) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form"
                                               href="javascript:confirmDelete('{{ route('admin.photo.destroy', ['id' => $row->id]) }}', '{{ adminNotify::CONFIRM_DELETE }}')">
