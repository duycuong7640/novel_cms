...
if (!defined('CURL_TIMEOUT')) define('CURL_TIMEOUT', 200);                            // Timeout duration for Curl. This only applies if you have Curl installed and aren't using PHP's default URL fetching mechanism.
...
if (!defined('DEFAULT_Q')) define('DEFAULT_Q', 0.3);                                    // Default image quality. Allows overrid in timthumb-config.php
...

{
...

    {
        ...
        if (isset($_GET['query'])) $str = explode('/', $arr[0]);
        ...
        if ($count) {
            ...
            $this->query['src'] = dirname($_SERVER['SCRIPT_NAME']) . '/' . $path . $img;
            ...
            if (in_array('sharpen', $str)) $this->query['s'] = 1;


            if (in_array('fill', $str)) $this->query['zc'] = 1;

            else if (in_array('fit', $str)) $this->query['zc'] = 2;

            else if (in_array('stretch', $str)) $this->query['zc'] = 0;

            else if (in_array('center', $str)) $this->query['zc'] = 4;

            else if (in_array('centre', $str)) $this->query['zc'] = 4;


            else $this->query['zc'] = 3;


            foreach ($str as $para) {

                if (preg_match('/^[hw][0-9]+$/i', $para)) $this->query[substr($para, 0, 1)] = substr($para, 1);

            }


            if (in_array('reverse', $str)) $filter[] = 1;

            if (in_array('bw', $str)) $filter[] = 2;


            if (in_array('edges', $str)) $filter[] = 6;

            if (in_array('emboss', $str)) $filter[] = 7;

            if (in_array('gaussian', $str)) $filter[] = 8;

            if (in_array('selective', $str)) $filter[] = 9;

            if (in_array('sketchy', $str)) $filter[] = 10;


            $this->query['f'] = implode('|', $filter);
...
//        echo '<br />TimThumb version : ' . VERSION . '</pre>';
...

    {
        ...
        if ($zoom_crop == 4) {
            ...
            imagecopyresampled($canvas, $image, 0, 0, $src_x, $src_y, $new_width, $new_height, $new_width, $new_height);
...

    {
        ...
        } else if (isset($this->query[$property])) {
